...
(function ($)
{
    ...
    $.ligerDefaults.Dialog = {
        ...
        content: '',    //内容
        myContent:'',   //自定义内容，为了兼容既有标题，又有内容的形式
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        _render: function ()
        {
            ...
            g.dialog.content = $(".l-dialog-content", g.dialog);
            ...
            if (p.buttons)
            {
                ...
                {
                    var btn = $('<a class="l-dialog-btn"><div class="l-dialog-btn-l"></div><div class="l-dialog-btn-r"></div><div class="l-dialog-btn-inner"></div></a>');
        ...
        {
            var g = this, p = this.options;
            if (g.trigger('Close',data) == false) return;
            g.doClose();
            if (g.trigger('Closed',data) == false) return;
        },
        doClose: function ()
        {
            var g = this;
        ...
        _setMyContent:function(content){//自定义内容
            
            $(".l-dialog-mycontent", this.dialog).html(content);
        },
        _setTitle: function (value)
        {
            ...
            if (value)
            {
                $(".l-dialog-title",g.dialog).find("i").html(value);
            }
            //console.info(p.type);
            if (p.type)
            {             
                if (p.type == 'error')
                {
                    $(".l-dialog-title",g.dialog).find(i).addClass("iconfont icon-cloud").show();
                }
                else if (p.type == 'warn')
                {
                    $(".l-dialog-title",g.dialog).find("i").addClass("iconfont icon-cloud").show();
                ...
                {
                    $(".l-dialog-title", g.dialog).addClass("l-dialog-title-image-question").show();
                }
            }
        },
        ...
        {
            var g = this, p = this.options;
            var alertCss = {paddingTop: 40,paddingBottom:15};
    ...
    $.ligerDialog.close = function ()
    {
     ...
     $.ligerDialog.closeId = function (id,data)
    {
        var g = this;
        var dialogs = l.find(l.controls.Dialog.prototype.__getType());
        for (var i in dialogs)
    ...
    $.ligerDialog.hideId = function (id)
    {
        var dialogs = l.find(l.controls.Dialog.prototype.__getType());
        for (var i in dialogs)
        {
    ...
    $.ligerDialog.alert = function (content, title, type, callback, options)
    {
        ...
        p = {
            content: content,
            buttons: [{ text: $.ligerDefaults.DialogString.ok, onclick: btnclick ,cls: 'btn btn-guide btnMr'}]
        };
     ...
     $.ligerDialog.confirm = function (tiptitle,title, width,callback,MyContent)
    {
        if (typeof (title) == "function")
        {
            callback = title;
    ...
    $.ligerDialog.prompt = function (title, value, multi, callback)
    {
        ...
        p = {
            ...
            target: target,
            width: 320,
            buttons: [{ text: $.ligerDefaults.DialogString.ok, onclick: btnclick, type: 'yes' },
             { text: $.ligerDefaults.DialogString.cancel, onclick: btnclick, type: 'cancel' }]
        };
        return $.ligerDialog(p);
    };
